export default function ActivityStateBar(){
    ...
    const activeUnassignedElement = (
        <div className="h-[151px] pt-[2px] flex flex-col items-end gap-4">
            <p className="text-subheading text-bright-blue">Unassigned</p>
    ...
    const activeAssignedElement = (
        <div className="h-[151px] pt-[2px] flex flex-col items-end gap-4">
            <p className="text-subheading text-bright-blue">Assigned</p>
    ...
    const inactiveUnassignedElement = (
        <div className="h-[68px]">
            <p className={`text-body ${statePriority[currentState] < statePriority["Unassigned"] ? "text-soft-gray" : "text-dark-blue"}`}>Unassigned</p>
        </div>
    );

    const inactiveAssignedElement = (
        <div className="h-[68px]">
            <p className={`text-body ${statePriority[currentState] < statePriority["Assigned"] ? "text-soft-gray" : "text-dark-blue"}`}>Assigned</p>
        </div>
    );
    ...
    const renderState = (checkedState: string, activeElement: JSX.Element, inactiveElement: JSX.Element) => {
        return currentState === checkedState ? activeElement : inactiveElement;
    }

    const renderDot = (checkedState: "Unassigned" | "Assigned" | "In progress" | "Returning" | "Completed") => {
    ...
    const renderLine = (checkedState: "Unassigned" | "Assigned" | "In progress" | "Returning" | "Completed") => {
